...
export function registerSessionCommands(
  ...
  registry: {
    getAgent: (id: string) => import("../../../application/orchestrator").AgentConfig | undefined;
  ...
  historyStore: { getEntries: () => Array<{
    sessionId: string; agentId: string; title: string; cwd: string;
    status: string; createdAt: string; updatedAt: string;
    messageCount: number; tokenUsage: { input: number; output: number; total: number };
  }>; clear: () => Promise<void> },
  resolveFile: (path: string, cwd?: string) => Promise<import("../../../domain/models/chat").ContextAttachmentDTO>,
  resolveSelection: () => Promise<import("../../../domain/models/chat").ContextAttachmentDTO | null>,
  resolveDiff: () => Promise<import("../../../domain/models/chat").ContextAttachmentDTO | null>,
  sendTabsToChatPanel: () => void,
): vscode.Disposable[] {
  // acp.newSession
  const newSessionCmd = vscode.commands.registerCommand("acp.newSession", async () => {
    // Step 1: choose action — new or fork
    const action = await vscode.window.showQuickPick(
      [
        { label: "$(add) New session", description: "Create a fresh session with a chosen agent and working directory" },
        { label: "$(git-fork) Fork session", description: "Clone an existing session from the current session list" },
      ],
    ...
    if (action.label.startsWith("$(git-fork)")) {
      // ---- Fork flow ----
      // Pick agent
      const agentId = await pickConnectedAgent("Select agent that owns the session to fork");
      if (!agentId) return;
      const sessions = orchestrator.getSessionsForAgent(agentId);
      if (sessions.length === 0) {
        void vscode.window.showWarningMessage("ACP: No sessions available to fork for this agent");
        return;
      }
      // Pick source session
      ...
      try {
        ...
        void vscode.window.showInformationMessage(`ACP: Forked session (${newId.slice(0, 8)})`);
      } catch (err) {
        void vscode.window.showErrorMessage(`ACP: Fork failed — ${err instanceof Error ? err.message : String(err)}`);
      }
    } else {
      // ---- New session flow ----
      ...
      type CwdItem = { label: string; description?: string; picked?: boolean; cwd: string };
      ...
      items.push({ label: "$(file-directory) Other…", description: "Enter a custom path", cwd: "" });
      ...
      let cwd = cwdPick.cwd;
      if (!cwd) {
        const fallback = wsFolders.length > 0 ? wsFolders[0].uri.fsPath : process.cwd();
      ...
      try {
        ...
        void vscode.window.showInformationMessage(`ACP: New session created (${sessionId.slice(0, 8)})`);
      } catch (err) {
        void vscode.window.showErrorMessage(`ACP: Failed to create session — ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  });

  // acp.switchSession
  const switchSessionCmd = vscode.commands.registerCommand("acp.switchSession", async () => {
  ...
  const cancelTurnCmd = vscode.commands.registerCommand("acp.cancelTurn", async () => {
    for (const agent of orchestrator.getAllAgents()) {
      for (const s of agent.sessions.filter((ss) => ss.status === "running")) {
  ...
  // acp.attachFile
  const attachFileCmd = vscode.commands.registerCommand("acp.attachFile", async (uri?: vscode.Uri) => {
    const activeAgent = orchestrator.getAllAgents()[0];
    const activeSessionId = activeAgent
      ? orchestrator.getActiveSessionId(activeAgent.agentId) ?? activeAgent.sessions[0]?.sessionId
      : undefined;
    const sessionInfo = activeAgent && activeSessionId
  ...
  // acp.attachSelection
  const attachSelectionCmd = vscode.commands.registerCommand("acp.attachSelection", async () => {
  ...
  // acp.attachDiff
  const attachDiffCmd = vscode.commands.registerCommand("acp.attachDiff", async () => {
  ...
  // acp.forkSession
  const forkSessionCmd = vscode.commands.registerCommand("acp.forkSession", async () => {
    const agentId = await pickConnectedAgent("Select agent");
    if (!agentId) return;
    const sessions = orchestrator.getSessionsForAgent(agentId);
    if (sessions.length === 0) return;
    const pick = await vscode.window.showQuickPick(
      sessions.map((s) => ({ label: s.title, description: s.sessionId.slice(0, 8), sessionId: s.sessionId })),
      { placeHolder: "Select session to fork" }
    );
    if (!pick) return;
    try {
  ...
  // acp.showHistory
  const showHistoryCmd = vscode.commands.registerCommand("acp.showHistory", async () => {
    ...
    if (!agentConfig) {
      void vscode.window.showErrorMessage(`ACP: Agent "${pick.entry.agentId}" not found in configuration`);
      return;
    }
    if (!orchestrator.getConnection(pick.entry.agentId)) {
      void vscode.window.showInformationMessage(`ACP: Agent "${agentConfig.name}" needs reconnection to load session`);
    }
    void vscode.window.showInformationMessage(`ACP: Session history entry selected (${pick.entry.sessionId.slice(0, 8)})`);
  });

  // acp.clearHistory
  const clearHistoryCmd = vscode.commands.registerCommand("acp.clearHistory", async () => {
    const confirm = await vscode.window.showWarningMessage("Clear all session history?", { modal: true }, "Clear");
  ...
  const closeSessionCmd = vscode.commands.registerCommand("acp.closeSession", async (sessionId?: string) => {
    if (!sessionId) {
      // No session id provided — pick from active agent
      const agentId = await pickConnectedAgent("Select agent that owns the session to close");
      if (!agentId) return;
      const sessions = orchestrator.getSessionsForAgent(agentId);
      if (sessions.length === 0) {
        void vscode.window.showWarningMessage("ACP: No sessions to close for this agent");
        return;
      }
      const pick = await vscode.window.showQuickPick(
        sessions.map((s) => ({
    ...
    void vscode.window.showWarningMessage(`ACP: Session "${sessionId}" not found`);
  });

  // acp.closeAllSessions
  const closeAllCmd = vscode.commands.registerCommand("acp.closeAllSessions", async () => {
    for (const agent of orchestrator.getAllAgents()) {
  ...
  // acp.showAgentMenu
  const showAgentMenuCmd = vscode.commands.registerCommand("acp.showAgentMenu", async () => {
    const agents = orchestrator.getAllAgents();
    const items: vscode.QuickPickItem[] = agents.length > 0
      ? [
          { label: "$(add) New Session", description: "Create a new session" },
          { label: "$(close) Close All Sessions", description: "Close all sessions" },
          { label: "$(output) Show Output", description: "Show agent output channel" },
          { label: "$(debug-disconnect) Disconnect", description: "Disconnect from agent" },
        ]
      : [
          { label: "$(plug) Connect to Agent", description: "Start an ACP agent connection" },
        ];
    const pick = await vscode.window.showQuickPick(items, { placeHolder: "ACP Agent Menu" });
    if (!pick) return;
    switch (pick.label) {
      case "$(add) New Session": void vscode.commands.executeCommand("acp.newSession"); break;
      case "$(close) Close All Sessions": void vscode.commands.executeCommand("acp.closeAllSessions"); break;
      case "$(output) Show Output": void vscode.window.showWarningMessage("ACP: showTraffic not yet implemented"); break;
      case "$(debug-disconnect) Disconnect": void vscode.commands.executeCommand("acp.disconnect"); break;
      case "$(plug) Connect to Agent": break; // handled by connect command
    }
  });
  ...
  async function disconnectCmd(agentId?: string): Promise<void> {
    ...
    const pick = await vscode.window.showQuickPick(
      agents.map((a) => ({ label: `$(hubot) ${a.agentId}`, description: a.state, aId: a.agentId })),
